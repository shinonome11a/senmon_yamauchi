...
int main(int argc, char* argv[]){
   ...
   while(1){
      ...
      recvline[n] = '\0';
      printf("%s:%d > ", inet_ntoa(ca.sin_addr),ntohs(ca.sin_port));
...
/*====================================================*/
      ...
      snprintf(sendline,BUFSIZE,"%s --> %s",inet_ntoa(ca.sin_addr),recvlarge);
      memset(recvline, '\0',BUFSIZE + 1);
      memset(recvlarge, '\0', BUFSIZE + 1);
      n = strlen(sendline);
      sendto(sockfd, sendline, n, 0, (struct sockaddr *)&sa, sizeof(sa));
      memset(sendline, '\0', BUFSIZE + 1);
   }
   return 0;
}
